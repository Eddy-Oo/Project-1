...
function listTasks(category) {
    // Code below this line vvvvv vvvvv vvvvv vvvvv vvvvv
    function finaloutput(task){
        return `${task.category}: ${task.task}`
    }
    function listbycategry(task){
        return task.category === category
    }


    if (category === 'all'){
        return tasks.map(finaloutput);  //some how this finaloutput function doesnt work on all catagories :"(
    }
    return tasks.filter(listbycategry).map(finaloutput)
